#!/usr/bin/env python3
"""
029_gene_density_sharper — sharpen weighting at the peak bin size.

Same as E28 (500kb, squared) but EPS=0.1 instead of 0.5.

Effect: bins with 0 genes get weight 0.01 (effectively zeroed),
bins with k genes get weight ~k^2. Pushes ~all mass onto
gene-dense bins; gene-deserts excluded.

E28 (EPS=0.5): 0.5086
This (EPS=0.1): ?

If > 0.5086: sharper concentration helps. Try EPS=0 next.
If ≈ 0.5086: saturated.
If < 0.5086: excluding gene-deserts removes useful diversity.
"""
import gzip
...
def main():
    ...
    weights /= weights.sum()
    print(f"{len(bin_weights):,} bins; top-50 mass: {sum(np.sort(weights)[-50:]):.4f}; zero-bin frac: {(weights[weights<1e-6]).sum():.4f}")
