"""025_exp12_plus_topic — exp 012 + DHS NMF-topic for eval_10 recovery.

Exp 012 family drops eval_10 (DHS chr7/13) by ~0.04. Adding 3k DHS-topic
(NMF-weighted) recovers eval_10 coverage while keeping cell-type DHS.

Composition (50k):
- 7k DHS Neural
- 5k DHS Myeloid/erythroid
- 5k DHS Digestive
- 3k DHS NMF-topic (for eval_10)
- 5k cCRE uniform
- 5k Table_S2
- 20k paired flanks
...
N_DHS_NEURAL = 7_000
N_DHS_MYELOID = 5_000
N_DHS_DIGESTIVE = 5_000
N_DHS_TOPIC = 3_000
N_CCRE_UNI = 5_000
N_S2 = 5_000
N_FLANK = 20_000
assert (N_DHS_NEURAL + N_DHS_MYELOID + N_DHS_DIGESTIVE + N_DHS_TOPIC
...
def sample_dhs_topic(seed, seen, n):
    """Sample n DHS sequences weighted by NMF topic mixture sum."""
    ...
    assert len(seqs) == n, f'topic only {len(seqs)}/{n}'
    return meta, seqs
...
def main():
    ...
    print(f'DHS topic {N_DHS_TOPIC}...', flush=True)
    m_t, s_t = sample_dhs_topic(SEED + 27, seen, N_DHS_TOPIC)
    ...
    fa = _fasta()
    pos_for_flanks = m_n + m_m + m_d + m_t + m_u
    ...
    all_seqs = s_n + s_m + s_d + s_t + s_u + s_s2 + s_f
