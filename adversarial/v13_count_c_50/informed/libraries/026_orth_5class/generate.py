"""
Experiment 026 — orth-DHS + cCRE 5 classes (no dELS, CA, CA-CTCF)

Test even more aggressive cCRE pruning. CA-CTCF is the largest of
remaining classes (126k). CTCF binding indicates insulator/boundary
function, not necessarily transcriptional activity.

If CTCF binding is less informative for MPRA activity than enhancer/
promoter elements, dropping it should give the remaining 5 classes
more budget each.

Design:
  25 k orth-DHS (numsamples ≤ 5)
  25 k cCRE balanced across 5 classes:
    CA-H3K4me3, CA-TF, PLS, TF, pELS (5000 each)
"""
...
def main():
    ...
    print(f"cCRE classes (excluding {EXCLUDE}): {classes}")
    per_cls = N_CCRE // len(classes)  # 5000
    parts = []
    for c in classes:
        cpool = np.where(cls_array == c)[0]
        sel = rng.choice(cpool, size=per_cls, replace=False) if len(cpool) >= per_cls else cpool.copy()
        parts.append(sel)
        print(f"  {c}: pool={len(cpool):,}, sampled={len(sel):,}")
