#!/usr/bin/env python3
"""
023_gene_density_cubic — push exponent further.

Weight = (gene_count + 0.5)^3, 1Mb bins. 3-seed.
...
If cubic > 0.510: monotonic. Push exponent.
If cubic ≈ squared: saturated; squared is the sweet spot.
If cubic < squared: over-narrowing kicks in.
"""
import gzip
...
def main():
    ...
    weights /= weights.sum()
    print(f"top fraction in top-5: {sum(np.sort(weights)[-5:]):.4f}")
    print(f"top fraction in top-50: {sum(np.sort(weights)[-50:]):.4f}")
