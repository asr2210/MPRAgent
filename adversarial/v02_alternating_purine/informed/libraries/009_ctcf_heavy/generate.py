"""009_ctcf_heavy — double CTCF count to amplify K562 unlock.

Theory v6 (compositional dominance): eval_06/11 (UKBB/GTEx K562) score
scales with CTCF count in library. Exp 007 had 4k CTCF -> eval_06/11=0.228.
If linear: 8k CTCF -> 0.25+?

Composition (50k):
- 8k cCRE uniform (vs exp 007's 10k)
- 8k cCRE CTCF-only (DOUBLED vs 4k)
- 4k cCRE DNase-H3K4me3 (held)
- 5k Table_S2 (held)
- 3k DHS-topic (vs exp 007's 4k)
- 2k synth (vs exp 007's 3k)
- 20k paired flanks (held)
"""
...
N_SYNTH = 2_000
N_FLANK = 20_000
assert N_CCRE_UNI + N_CTCF + N_DNH3 + N_S2 + N_DHS + N_SYNTH + N_FLANK == 50_000
...
def main():
    ...
    all_seqs = s_u + s_c + s_d + s_s2 + s_dh + s_sy + s_f
