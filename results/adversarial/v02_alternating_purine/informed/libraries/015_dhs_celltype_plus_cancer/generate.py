"""015_dhs_celltype_plus_cancer — add Cancer/epithelial DHS component.

All 3 model cell types (K562/HepG2/SK-N-SH) are cancer-derived. Adding DHS
from the "Cancer / epithelial" component might lift all 3 heads via shared
oncogenic regulatory signature.

ORIGINAL EXP 012 DOCSTRING BELOW:
...
N_S2 = 4_000
N_FLANK = 20_000
assert (N_DHS_NEURAL + N_DHS_MYELOID + N_DHS_DIGESTIVE + N_DHS_CANCER
...
def main():
    ...
    print(f'DHS Cancer/epithelial {N_DHS_CANCER}...', flush=True)
    m_ca, s_ca = sample_dhs_component(dhs, 'Cancer / epithelial', N_DHS_CANCER, SEED + 24, seen)
    ...
    fa = _fasta()
    pos_for_flanks = m_n + m_m + m_d + m_ca + m_u
    ...
    all_seqs = s_n + s_m + s_d + s_ca + s_u + s_s2 + s_f
