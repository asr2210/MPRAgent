"""
Experiment 020: High-density motif packing with overlap allowed.

Insert 8 motifs per sequence, OVERLAP ALLOWED. Each insertion can overwrite
earlier ones. The result is a sequence dense with motif-like content.

This is qualitatively different from 008-family (3 non-overlapping motifs).
Tests whether saturating the sequence with motif content boosts TF presence
signal, even at the cost of motif fidelity (overwriting earlier motifs).
"""
...
def main() -> None:
    ...
    for i in range(N_TOTAL):
        ...
        # Insert N_MOTIFS with overlap allowed (each overwrites freely)
