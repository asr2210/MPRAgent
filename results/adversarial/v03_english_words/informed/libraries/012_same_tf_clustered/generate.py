"""
Experiment 012: Same-TF clustering — 3 stochastic samples of ONE TF per seq.

Same 289-motif pool as exp 008. Difference: each sequence picks ONE TF and
inserts 3 stochastic PFM-samples of it (vs 3 different TFs in exp 008).
Tests whether the surrogate uses motif "density per TF" as a feature beyond
mere presence/absence.

Outcomes:
  - If exp 012 > 0.4283: per-TF clustering amplifies signal; surrogate uses
    density per TF as a feature → motif "stamps" matter more than diversity.
  - If exp 012 < 0.4283: motif diversity within a sequence beats per-TF
    repetition → T8/T9 confirmed (information density needs many distinct
    motifs, not many copies of one).
"""
...
def main() -> None:
    ...
    for i in range(N_TOTAL):
        ...
        # Pick ONE TF for this sequence
        tf_idx = int(rng.integers(0, len(pfms)))
        _, pfm = pfms[tf_idx]
        mlen = pfm.shape[1]
