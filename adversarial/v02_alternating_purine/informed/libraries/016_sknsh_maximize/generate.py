"""016_sknsh_maximize — push SKNSH r ceiling.

Since K562/HepG2 r stuck near 0 across recipes, lever the dominant SKNSH r
which has headroom (current ~0.45, theoretical ~0.55+).

Composition:
- 12k DHS Neural (SKNSH-tissue match, BOOST)
- 5k DHS Tissue-invariant (constitutive high activity)
- 4k cCRE PLS (promoters, broad activity)
- 4k cCRE uniform
- 4k Table_S2 random
- 21k paired flanks (more negatives for contrast)

ORIGINAL DOC:
DHS enriched for the 3 model cell types.
...
N_S2 = 4_000
N_FLANK = 21_000
assert (N_DHS_NEURAL + N_DHS_TINV + N_CCRE_PLS
...
def main():
    ...
    print(f'DHS Tissue-invariant {N_DHS_TINV}...', flush=True)
    m_t, s_t = sample_dhs_component(dhs, 'Tissue invariant', N_DHS_TINV, SEED + 25, seen)
    print(f'cCRE PLS {N_CCRE_PLS}...', flush=True)
    m_p, s_p = sample_class(ccres, 'PLS', N_CCRE_PLS, SEED + 1, seen)
    ...
    fa = _fasta()
    pos_for_flanks = m_n + m_t + m_p + m_u
    ...
    all_seqs = s_n + s_t + s_p + s_u + s_s2 + s_f
