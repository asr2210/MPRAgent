"""
Experiment 016: Re-run of exp 008 with SEED=42 instead of 0.

Same library design as exp 008 (best so far, mean_r=0.4283). Only the
random seed changes. The point is to estimate library noise: how much
does mean_r vary when the only thing different is the random draws of
motif positions and PFM samples?

If exp 016 lands within ±0.001 of 008: 008's edge over recent
experiments (010-015) is real signal, and I should keep exploring radically
different designs.

If exp 016 lands ±0.01 from 008: my "regressions" might all be noise,
and I should run multi-seed evaluations to make claims.
"""
...
def main() -> None:
    ...
    print(f"Loaded {len(pfms)} targeted PFMs (SEED={SEED})")
