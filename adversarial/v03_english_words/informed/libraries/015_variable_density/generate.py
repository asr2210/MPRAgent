"""
Experiment 015: Variable motif density (1-6 motifs/seq).

Same 289-PFM pool as exp 008 (best, 0.4283). Each sequence draws its motif
count uniformly from [1, 6] instead of fixed at 3. Tests whether density
diversity improves training:
- Exp 008 (fixed 3): 0.4283
- Exp 009 (fixed 5): 0.4259
- Exp 015 (1-6 uniform mix): ?

If density mixing > fixed-3: training-distribution coverage matters.
If <: the surrogate prefers consistent density input.
"""
...
def main() -> None:
    ...
    for i in range(N_TOTAL):
        ...
        densities.append(n_motifs)
        positions_taken: list[tuple[int, int]] = []
        inserted = 0
        attempts = 0
        while inserted < n_motifs and attempts < 120:
            attempts += 1
    ...
    print(f"Density distribution: mean={densities.mean():.2f}, "
          f"counts={[int((densities == k).sum()) for k in range(MIN_MOTIFS, MAX_MOTIFS + 1)]}")
